...
export const ObjectToMove = ({ objToRender }) => {
  ...
  useEffect(() => {
    ...
    // Limit controls to the size of the container
    controls.maxDistance = Math.min(width, height) / 2;
    ...
    const animate = () => {
      ...
      if (object) {
        object.rotation.x += 0.005; // Automatic rotation on the x-axis
        object.rotation.y += 0.005; // Automatic rotation on the y-axis
      }
